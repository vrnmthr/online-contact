...
const Classic = (props) => {
  ...
  //represnts the current cluemaster
  ...
  const [curr_clue, setCurrClue] = useState({});
  ...
  useEffect(() => {
    ...
    socket.on("start_turn", (args) => {
      ...
      setcur_cluemaster({ name: args.name.name, id: args.id });
    });

    //gets teh game_word
    socket.on("start_phase", (args) => {
      setgame_word(args.word);
      setshowClue(args.show_clue);
    });
    ...
    //if one render letter and progress for not cluemaster
      ...
      if (name === cur_cluemaster.name) {
        ...
        if (game_word.word !== undefined && game_word.progress !== undefined) {
          ...
          else{for (let i = 0; i < game_word.progress+1; i++) {
